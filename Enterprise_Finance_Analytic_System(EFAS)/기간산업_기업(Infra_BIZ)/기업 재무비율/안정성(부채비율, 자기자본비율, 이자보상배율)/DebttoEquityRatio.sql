 ...
 * 활용 테이블 : GIUP_RAW, TCB_NICE_FNST(재무제표) -> DEBT_RATIO_TB 테이블 만듦
 *****************************************************/   
-- (Standby) 월별, 산업별, 기업규모별 부채비율 테이블 생성 
-- 과거 데이터가 있으면 table 삭제하고 새로 생성
DROP TABLE IF EXISTS DEBT_RATIO_TB;
-- 테이블 생성 (기준년월, 법인번호, 기업규모, KSIC, 부채, 총자산, EFAS)
SELECT 
	t10.*, 
	t20.EFIS as EFAS
	INTO DEBT_RATIO_TB 
...
SELECT 
  	...
  	SUM(DECODE(t0.STD_YM, CONCAT('${inputYYYY}' - 1, '12'), EFAS_DEBT, 0)) as prevYY_EFAS_DEBT, 
  	SUM(DECODE(t0.STD_YM, CONCAT('${inputYYYY}' - 1, '12'), EFAS_CAPITAL, 0)) as prevYY_EFAS_CAPITAL 
