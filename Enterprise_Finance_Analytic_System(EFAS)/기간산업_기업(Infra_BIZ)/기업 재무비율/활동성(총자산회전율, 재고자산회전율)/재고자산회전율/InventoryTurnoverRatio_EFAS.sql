...
-- (기준년월, 법인번호, 기업규모, 외감여부, 상장여부, KSIC, 결산구분, 매출액, 재고자산, 재고자산회전률, EFAS)
SELECT 
	t0.STD_YM,
	t0.CORP_NO,
	t0.COMP_SCL_DIVN_CD,
...
SELECT 	
	t000.*,
	thisYY_EFAS_INVENTORY_TURNOVER_RATIO - prevYY_EFAS_INVENTORY_TURNOVER_RATIO as "INCRATIO"	-- 전년 동월 대비 증감율
FROM 
	(
	SELECT DISTINCT
		t00.EFAS,
		t00.thisYY,
		SUM(t00.thisYY_EFAS_INVENTORY_TURNOVER_RATIO) OVER(PARTITION BY t00.EFAS) as thisYY_EFAS_INVENTORY_TURNOVER_RATIO,	-- 현월 재고자산 회전률
		SUM(t00.prevYY_EFAS_INVENTORY_TURNOVER_RATIO) OVER(PARTITION BY t00.EFAS) as prevYY_EFAS_INVENTORY_TURNOVER_RATIO	-- 전년 동월 재고자산 회전률
	FROM
		(	
		SELECT 
			t0.EFAS,
		  	...
		  	END as "thisYY_EFAS_INVENTORY_TURNOVER_RATIO",
		  	CASE	
		  		WHEN '${isSangJang}' = 'Y' 
		  	...
		  	END as "prevYY_EFAS_INVENTORY_TURNOVER_RATIO"
		FROM
			(
			SELECT DISTINCT 
				t.EFAS,
