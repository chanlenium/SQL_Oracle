...
SELECT 	
	t000.*,
	thisYY_EFAS_ASSET_TURNOVER_RATIO - prevYY_EFAS_ASSET_TURNOVER_RATIO as "INCRATIO"	-- 전년 동월 대비 증감율
FROM 
	(
	SELECT DISTINCT
		t00.EFAS,
		t00.thisYY,
		SUM(t00.thisYY_EFAS_ASSET_TURNOVER_RATIO) OVER(PARTITION BY t00.EFAS) as thisYY_EFAS_ASSET_TURNOVER_RATIO,	-- 현월 총자산회전률
		SUM(t00.prevYY_EFAS_ASSET_TURNOVER_RATIO) OVER(PARTITION BY t00.EFAS) as prevYY_EFAS_ASSET_TURNOVER_RATIO	-- 전년 동월 총자산회전률
	FROM
		(	
		SELECT 
			t0.EFAS,
		  	...
		  	END as "thisYY_EFAS_ASSET_TURNOVER_RATIO",
		  	CASE	
		  		WHEN '${isSangJang}' = 'Y' 
		  	...
		  	END as "prevYY_EFAS_ASSET_TURNOVER_RATIO"
		FROM
			(
			SELECT DISTINCT 
				t.EFAS,
