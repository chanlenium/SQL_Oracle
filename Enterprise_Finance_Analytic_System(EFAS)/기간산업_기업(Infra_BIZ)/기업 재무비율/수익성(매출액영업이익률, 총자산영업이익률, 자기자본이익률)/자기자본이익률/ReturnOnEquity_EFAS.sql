...
SELECT 	
	t000.*,
	thisYY_EFAS_RETURN_ON_EQUITY_RATIO - prevYY_EFAS_RETURN_ON_EQUITY_RATIO as "INCRATIO"	-- 전년 동월 대비 증감율
FROM 
	(
	SELECT DISTINCT
		t00.EFAS,
		t00.thisYY,
		SUM(t00.thisYY_EFAS_RETURN_ON_EQUITY_RATIO) OVER(PARTITION BY t00.EFAS) as thisYY_EFAS_RETURN_ON_EQUITY_RATIO,	-- 현월 자기자본이익률
		SUM(t00.prevYY_EFAS_RETURN_ON_EQUITY_RATIO) OVER(PARTITION BY t00.EFAS) as prevYY_EFAS_RETURN_ON_EQUITY_RATIO	-- 전년 동월 자기자본이익률
	FROM
		(	
		SELECT 
			t0.EFAS,
		  	...
		  	END as "thisYY_EFAS_RETURN_ON_EQUITY_RATIO",
		  	CASE	
		  		WHEN '${isSangJang}' = 'Y' 
		  	...
		  	END as "prevYY_EFAS_RETURN_ON_EQUITY_RATIO"
		FROM
			(
			SELECT DISTINCT 
				t.EFAS,
