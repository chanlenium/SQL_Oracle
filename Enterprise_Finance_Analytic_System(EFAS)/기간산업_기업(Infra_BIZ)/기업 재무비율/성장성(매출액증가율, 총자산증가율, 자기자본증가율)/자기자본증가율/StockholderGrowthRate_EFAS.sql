 ...
 * 활용 테이블 : GIUP_RAW, TCB_NICE_FNST(재무제표) -> STOCKHOLDERS_TB 테이블 만듦
 ****************************************************************/
-- (Standby) 월별, 산업별, 기업규모별 자기자본 테이블 STOCKHOLDERS_TB 생성 
-- 과거 데이터가 있으면 table 삭제하고 새로 생성
DROP TABLE IF EXISTS STOCKHOLDERS_TB;
-- (Step1) 테이블 생성(기준년월, 법인번호, 기업규모, 외감여부, 상장구분코드, KSIC, 결산구분, 자기자본, EFAS)
SELECT 
	t0.STD_YM,
	t0.CORP_NO,
	t0.COMP_SCL_DIVN_CD,
...
SELECT 	
	t000.*,
	thisYY_EFAS_STOCKHOLDERS_INC_RATIO - prevYY_EFAS_STOCKHOLDERS_INC_RATIO as "INCRATIO"	-- 전년 동월 대비 증감율
FROM 
	(
	SELECT DISTINCT
		t00.EFAS,
		t00.thisYY,
		SUM(t00.thisYY_EFAS_STOCKHOLDERS_INC_RATIO) OVER(PARTITION BY t00.EFAS) as thisYY_EFAS_STOCKHOLDERS_INC_RATIO,	-- 현월 자기자본증감률
		SUM(t00.prevYY_EFAS_STOCKHOLDERS_INC_RATIO) OVER(PARTITION BY t00.EFAS) as prevYY_EFAS_STOCKHOLDERS_INC_RATIO	-- 전년 동월 자기자본증감률
	FROM
		(	
		SELECT 
			t0.EFAS,
		  	...
		  	END as "thisYY_EFAS_STOCKHOLDERS_INC_RATIO",
		  	CASE	
		  		WHEN '${isSangJang}' = 'Y' 
		  	...
		  	END as "prevYY_EFAS_STOCKHOLDERS_INC_RATIO"
		FROM
			(
			SELECT DISTINCT 
				t.EFAS,
