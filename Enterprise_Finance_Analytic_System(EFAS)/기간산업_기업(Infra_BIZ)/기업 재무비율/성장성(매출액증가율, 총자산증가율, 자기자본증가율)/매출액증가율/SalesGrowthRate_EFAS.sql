 ...
 * 활용 테이블 : GIUP_RAW, TCB_NICE_FNST(재무제표) -> SALES_TB 테이블 만듦
 ****************************************************************/
...
SELECT 	
	t000.*,
	thisYY_EFAS_SALES_INC_RATIO - prevYY_EFAS_SALES_INC_RATIO as "INCRATIO"	-- 전년 동월 대비 증감율
FROM 
	(
	SELECT DISTINCT
		t00.EFAS,
		t00.thisYY,
		SUM(t00.thisYY_EFAS_SALES_INC_RATIO) OVER(PARTITION BY t00.EFAS) as thisYY_EFAS_SALES_INC_RATIO,	-- 현월 매출액성장률
		SUM(t00.prevYY_EFAS_SALES_INC_RATIO) OVER(PARTITION BY t00.EFAS) as prevYY_EFAS_SALES_INC_RATIO	-- 전년 동월 매출액성장률
	FROM
		(	
		SELECT 
			t0.EFAS,
