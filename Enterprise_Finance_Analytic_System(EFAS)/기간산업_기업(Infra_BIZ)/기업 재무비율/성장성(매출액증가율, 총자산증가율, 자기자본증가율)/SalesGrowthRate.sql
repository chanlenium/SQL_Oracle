...
-- 테이블 생성(기준년월, 법인번호, 기업규모, KSIC, 매출액, EFAS)
SELECT 
  	t10.*, 
  	t20.EFIS as EFAS
  	INTO SALES_TB 
...
FROM
	...
	SELECT DISTINCT 
		t0.EFAS,  
  		SUM(t0.SALES_N3) OVER(PARTITION BY t0.EFAS) as TOT_SALES_N3, 
  		SUM(t0.SALES_N2) OVER(PARTITION BY t0.EFAS) as TOT_SALES_N2,
  		SUM(t0.SALES_N1) OVER(PARTITION BY t0.EFAS) as TOT_SALES_N1,
  		SUM(t0.SALES_N0) OVER(PARTITION BY t0.EFAS) as TOT_SALES_N0
	FROM 
    	(
    	SELECT DISTINCT
			t.CORP_NO,
