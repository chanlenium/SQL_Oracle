...
FROM
	(
	SELECT DISTINCT	-- 사업자 대출 case (사업자, 가계 교집합 포함)
		t0.GG_YM,
	...
	FROM
		...
		WHERE 
			...
			AND NVL(t.BRNO_AMT, 0) <> 0
		) t0
	) t1,
	(
	SELECT DISTINCT	-- 가계 대출 case (사업자, 가계 교집합 포함)
		t0.GG_YM,
		dense_Rank() over (partition by t0.GG_YM order by t0.CORP_NO) 
			+ dense_Rank() over (partition by t0.GG_YM order by t0.CORP_NO desc) - 1 as noOwner_HOULoan,
		dense_Rank() over (partition by t0.GG_YM order by t0.BRNO) 
			+ dense_Rank() over (partition by t0.GG_YM order by t0.BRNO desc) - 1 as noBIZ_HOULoan,
		ROUND(SUM(NVL(t0.BRNO_AMT, 0) + NVL(t0.HOU_LOAN, 0) / 1000) OVER(PARTITION BY t0.GG_YM), 0) as AMT_HOULoan
	FROM
		(
		SELECT 
			t.GG_YM,
	...
	SELECT DISTINCT	-- 사업자 & 가계 대출 모두 있는 case (교집합)
		t0.GG_YM,
		dense_Rank() over (partition by t0.GG_YM order by t0.CORP_NO) 
			+ dense_Rank() over (partition by t0.GG_YM order by t0.CORP_NO desc) - 1 as noOwner_BIZHOULoan,
		dense_Rank() over (partition by t0.GG_YM order by t0.BRNO) 
			+ dense_Rank() over (partition by t0.GG_YM order by t0.BRNO desc) - 1 as noBIZ_BIZHOULoan,
		ROUND(SUM(NVL(t0.BRNO_AMT, 0) + NVL(t0.HOU_LOAN, 0) / 1000) OVER(PARTITION BY t0.GG_YM), 0) as AMT_BIZHOULoan
	FROM
		(
		SELECT 
			t.GG_YM,
...
-- 결과 조회
SELECT * FROM RESULT_INDBIZLOAN;
SELECT * FROM RESULT_INDBIZLOAN_EFAS;
SELECT * FROM RESULT_INDBIZLOAN_UPREOK;
SELECT * FROM RESULT_INDBIZLOAN_LOCATION;
