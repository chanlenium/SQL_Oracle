...
-- (Step4) 총자산회전율 연별 그래프 (대/중/소) (1: 대기업, 2: 중소기업, 3: 중견기업, 0: 대상아님)
DROP TABLE IF EXISTS Result_newINDU_AssetTurnoverRatio_BIZSIZE;
SELECT DISTINCT
	t00.STD_YM,
	SUM(ASSET_TURNOVER_RATIO_BIZ_1) OVER(PARTITION BY t00.STD_YM) as ASSET_TURNOVER_RATIO_BIZ_1,
	SUM(ASSET_TURNOVER_RATIO_BIZ_2) OVER(PARTITION BY t00.STD_YM) as ASSET_TURNOVER_RATIO_BIZ_2,
	SUM(ASSET_TURNOVER_RATIO_BIZ_3) OVER(PARTITION BY t00.STD_YM) as ASSET_TURNOVER_RATIO_BIZ_3
	INTO Result_newINDU_AssetTurnoverRatio_BIZSIZE
FROM
	(	
	SELECT 
		t0.*,
		DECODE(t0.BIZ_SIZE, '대기업', t0.ASSET_TURNOVER_RATIO, 0) as ASSET_TURNOVER_RATIO_BIZ_1,
		DECODE(t0.BIZ_SIZE, '중소기업', t0.ASSET_TURNOVER_RATIO, 0) as ASSET_TURNOVER_RATIO_BIZ_2,
		DECODE(t0.BIZ_SIZE, '중견기업', t0.ASSET_TURNOVER_RATIO, 0) as ASSET_TURNOVER_RATIO_BIZ_3
	FROM
		(
		SELECT DISTINCT 
			CASE
...
ORDER BY t00.STD_YM;
-- 결과 조회
SELECT * FROM Result_newINDU_AssetTurnoverRatio_BIZSIZE ORDER BY STD_YM;
