...
-- (Step4) 재고자산회전율 연별 그래프 (업종별)
DROP TABLE IF EXISTS Result_newINDU_IventoryTurnoverRatio_INDU;
SELECT 	
	t000.*,
	thisYY_newINDU_INVENTORY_TURNOVER_RATIO - prevYY_newINDU_INVENTORY_TURNOVER_RATIO as "INCRATIO"	-- 전년 동월 대비 증감율
	INTO Result_newINDU_IventoryTurnoverRatio_INDU
FROM 
	(
	SELECT DISTINCT
		t00.newINDU,
		t00.thisYY,
		SUM(t00.thisYY_newINDU_INVENTORY_TURNOVER_RATIO) OVER(PARTITION BY t00.newINDU) as thisYY_newINDU_INVENTORY_TURNOVER_RATIO,	-- 현월 재고자산 회전률
		SUM(t00.prevYY_newINDU_INVENTORY_TURNOVER_RATIO) OVER(PARTITION BY t00.newINDU) as prevYY_newINDU_INVENTORY_TURNOVER_RATIO	-- 전년 동월 재고자산 회전률
	FROM
		(	
		SELECT 
			t0.newINDU,
		  	...
		  	CASE	
		  		WHEN '${isSangJang}' = 'Y' 
		  		THEN DECODE(t0.STD_YM, t0.thisYY, t0.INVENTORY_TURNOVER_RATIO, 0)
		  		ELSE DECODE(CONCAT(t0.STD_YM, '12'), t0.thisYY, t0.INVENTORY_TURNOVER_RATIO, 0)
		  	END as "thisYY_newINDU_INVENTORY_TURNOVER_RATIO",
		  	CASE	
		  		WHEN '${isSangJang}' = 'Y' 
		  		THEN DECODE(t0.STD_YM, t0.prevYY, t0.INVENTORY_TURNOVER_RATIO, 0)
		  		ELSE DECODE(CONCAT(t0.STD_YM, '12'), t0.prevYY, t0.INVENTORY_TURNOVER_RATIO, 0)
		  	END as "prevYY_newINDU_INVENTORY_TURNOVER_RATIO"
		FROM
			(
			SELECT DISTINCT 
				t.newINDU,
...
-- 결과 조회
SELECT * FROM Result_newINDU_IventoryTurnoverRatio_INDU ORDER BY TO_NUMBER(newINDU, '99');
