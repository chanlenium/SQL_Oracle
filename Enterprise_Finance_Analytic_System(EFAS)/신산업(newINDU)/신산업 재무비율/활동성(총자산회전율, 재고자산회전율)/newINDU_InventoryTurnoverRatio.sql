...
-- 테이블 생성 (기준년월, 법인번호, 기업규모, KSIC, newINDU, newINDU_NM, 매출액, 재고자산(계), EFAS)
SELECT 
  	t10.*, 
  	t20.EFIS as EFAS
...
FROM 
	...
	SELECT DISTINCT 
		...
		t.INVENTORY
	FROM 
		INVENTORY_TURNOVER_RATIO_TB t	-- 동일 법인번호에 newINDU가 여러개 매핑되는 경우 재무규모가 과대개상될 수 있어 이를 제거
	) t
ORDER BY 
  	t.STD_YM;
...
/****************************************************************
 * 자기자본비율 : 업종별 현황 (p.6, [표])
 * 활용 테이블 : INVENTORY_TURNOVER_RATIO_TB -> INDU_INVENTORY_TURNOVER_RATIO_TB 생성
 ****************************************************************/
-- 과거 데이터가 있으면 table 삭제하고 새로 생성
