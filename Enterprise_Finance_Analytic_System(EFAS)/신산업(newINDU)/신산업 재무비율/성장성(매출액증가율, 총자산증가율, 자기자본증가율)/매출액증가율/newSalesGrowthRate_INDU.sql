...
--(Step2) 실질매출액을 구함 : 상장기업의 경우 직전분기 매출액 차분값으로 실질매출액 도출
DROP TABLE IF EXISTS EFFECTIVE_SALES_TB;
-- 테이블 생성(기업규모, 기준년월, 당해 매출액, 직전 매출액, 실질매출액)
SELECT DISTINCT
	t00.STD_YM,
	t00.prevSTD_YM,
	t00.CORP_NO,
...
-- (Step3) 실질 매출실적(effSALES)을 활용하여 매출액 성장률 도출
-- 과거 데이터가 있으면 table 삭제하고 새로 생성
...
-- (Step6) 매출액 성장률 그래프 (업종별)
DROP TABLE IF EXISTS Result_newINDU_SalesGrowthRate_INDU;
SELECT 	
	t000.*,
	thisYY_newINDU_SALES_INC_RATIO - prevYY_newINDU_SALES_INC_RATIO as "INCRATIO"	-- 전년 동월 대비 증감율
	INTO Result_newINDU_SalesGrowthRate_INDU
FROM 
	(
	SELECT DISTINCT
		t00.newINDU,
		t00.thisYY,
		SUM(t00.thisYY_newINDU_SALES_INC_RATIO) OVER(PARTITION BY t00.newINDU) as thisYY_newINDU_SALES_INC_RATIO,	-- 현월 매출액성장률
		SUM(t00.prevYY_newINDU_SALES_INC_RATIO) OVER(PARTITION BY t00.newINDU) as prevYY_newINDU_SALES_INC_RATIO	-- 전년 동월 매출액성장률
	FROM
		(	
		SELECT 
			t0.newINDU,
		  	...
		  	CASE	
		  		WHEN '${isSangJang}' = 'Y' 
		  		THEN DECODE(t0.STD_YM, t0.thisYY, t0.SALES_INC_RATIO, 0)
		  		ELSE DECODE(CONCAT(t0.STD_YM, '12'), t0.thisYY, t0.SALES_INC_RATIO, 0)
		  	END as "thisYY_newINDU_SALES_INC_RATIO",
		  	CASE	
		  		WHEN '${isSangJang}' = 'Y' 
		  		THEN DECODE(t0.STD_YM, t0.prevYY, t0.SALES_INC_RATIO, 0)
		  		ELSE DECODE(CONCAT(t0.STD_YM, '12'), t0.prevYY, t0.SALES_INC_RATIO, 0)
		  	END as "prevYY_newINDU_SALES_INC_RATIO"
		FROM
			(
			SELECT DISTINCT 
				t.newINDU,
...
-- 결과조회
SELECT * FROM Result_newINDU_SalesGrowthRate_INDU ORDER BY TO_NUMBER(newINDU, '99');
