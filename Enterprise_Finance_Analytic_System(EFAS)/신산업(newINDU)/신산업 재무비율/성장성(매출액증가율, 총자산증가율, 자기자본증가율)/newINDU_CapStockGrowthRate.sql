 ...
 * 활용 테이블 : newGIUP_RAW, TCB_NICE_FNST(재무제표) -> CapSTOCK_TB 테이블 만듦
 ****************************************************************/
	...
	FROM 
		...
		FROM 
			...
			SELECT DISTINCT 
				...
				t.CapSTOCK 
			FROM 
				CapSTOCK_TB t	-- 동일 법인번호에 newINDU가 여러개 매핑되는 경우 재무규모가 과대개상될 수 있어 이를 제거
			) t	
	  	) t0
	) t00;
...
FROM
	...
	SELECT DISTINCT 
		...
		t0.newINDU_NM,
  		SUM(t0.CapSTOCK_N3) OVER(PARTITION BY t0.newINDU) as TOT_CapSTOCK_N3, 
  		SUM(t0.CapSTOCK_N2) OVER(PARTITION BY t0.newINDU) as TOT_CapSTOCK_N2,
  		SUM(t0.CapSTOCK_N1) OVER(PARTITION BY t0.newINDU) as TOT_CapSTOCK_N1,
  		SUM(t0.CapSTOCK_N0) OVER(PARTITION BY t0.newINDU) as TOT_CapSTOCK_N0
	FROM 
    	(
    	SELECT DISTINCT
			t.CORP_NO,
