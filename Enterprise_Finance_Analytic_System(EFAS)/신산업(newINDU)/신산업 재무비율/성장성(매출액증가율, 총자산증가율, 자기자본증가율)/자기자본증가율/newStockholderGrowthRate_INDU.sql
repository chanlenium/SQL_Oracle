...
-- 결과 조회
SELECT * FROM STOCKHOLDERS_TB ORDER BY STD_YM;
...
-- (Step3) 실질 매출실적(effSTOCKHOLDERS)을 활용하여 자기자본 증감률 도출
-- 과거 데이터가 있으면 table 삭제하고 새로 생성
...
-- (Step6) 자기자본 증감률 연별 그래프 (업종별)
DROP TABLE IF EXISTS Result_newINDU_STOCKHOLDERSGrowthRate_INDU;
SELECT 	
	t000.*,
	thisYY_newINDU_STOCKHOLDERS_INC_RATIO - prevYY_newINDU_STOCKHOLDERS_INC_RATIO as "INCRATIO"	-- 전년 동월 대비 증감율
	INTO Result_newINDU_STOCKHOLDERSGrowthRate_INDU
FROM 
	(
	SELECT DISTINCT
		t00.newINDU,
		t00.thisYY,
		SUM(t00.thisYY_newINDU_STOCKHOLDERS_INC_RATIO) OVER(PARTITION BY t00.newINDU) as thisYY_newINDU_STOCKHOLDERS_INC_RATIO,	-- 현월 자기자본증감률
		SUM(t00.prevYY_newINDU_STOCKHOLDERS_INC_RATIO) OVER(PARTITION BY t00.newINDU) as prevYY_newINDU_STOCKHOLDERS_INC_RATIO	-- 전년 동월 자기자본증감률
	FROM
		(	
		SELECT 
			t0.newINDU,
		  	...
		  	CASE	
		  		WHEN '${isSangJang}' = 'Y' 
		  		THEN DECODE(t0.STD_YM, t0.thisYY, t0.STOCKHOLDERS_INC_RATIO, 0)
		  		ELSE DECODE(CONCAT(t0.STD_YM, '12'), t0.thisYY, t0.STOCKHOLDERS_INC_RATIO, 0)
		  	END as "thisYY_newINDU_STOCKHOLDERS_INC_RATIO",
		  	CASE	
		  		WHEN '${isSangJang}' = 'Y' 
		  		THEN DECODE(t0.STD_YM, t0.prevYY, t0.STOCKHOLDERS_INC_RATIO, 0)
		  		ELSE DECODE(CONCAT(t0.STD_YM, '12'), t0.prevYY, t0.STOCKHOLDERS_INC_RATIO, 0)
		  	END as "prevYY_newINDU_STOCKHOLDERS_INC_RATIO"
		FROM
			(
			SELECT DISTINCT 
				t.newINDU,
...
-- 결과 조회
SELECT * FROM Result_newINDU_STOCKHOLDERSGrowthRate_INDU ORDER BY TO_NUMBER(newINDU, '99');
