...
DROP TABLE IF EXISTS Result_newINDU_AssetsGrowthRate_INDU;
SELECT 	
	t000.*,
	thisYY_newINDU_ASSETS_INC_RATIO - prevYY_newINDU_ASSETS_INC_RATIO as "INCRATIO"	-- 전년 동월 대비 증감율
	INTO Result_newINDU_AssetsGrowthRate_INDU
FROM 
	(
	SELECT DISTINCT
		t00.newINDU,
		t00.thisYY,
		SUM(t00.thisYY_newINDU_ASSETS_INC_RATIO) OVER(PARTITION BY t00.newINDU) as thisYY_newINDU_ASSETS_INC_RATIO,	-- 현월 총자산증감률
		SUM(t00.prevYY_newINDU_ASSETS_INC_RATIO) OVER(PARTITION BY t00.newINDU) as prevYY_newINDU_ASSETS_INC_RATIO	-- 전년 동월 총자산증감률
	FROM
		(	
		SELECT 
			t0.newINDU,
...
-- 결과 조회
SELECT * FROM Result_newINDU_AssetsGrowthRate_INDU ORDER BY TO_NUMBER(newINDU, '99');
