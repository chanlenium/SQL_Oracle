...
-- 테이블 생성 (기준년월, 법인번호, 기업규모, 외감여부, 상장여부, KSIC, 신산업코드, 신산업명, 결산구분, 자기자본(자본총계), 총자산, 자기자본비율, EFAS)
SELECT 
	t0.STD_YM,
	t0.CORP_NO,
	t0.COMP_SCL_DIVN_CD,
...
SELECT 	
	t000.*,
	thisYY_newINDU_CapitalAdequacyRatio - prevYY_newINDU_CapitalAdequacyRatio as "INCRATIO"	-- 전년 동월 대비 증감율
...
FROM 
	...
	SELECT DISTINCT
		t00.newINDU,
		t00.thisYY,
		SUM(t00.thisYY_CapitalAdequacyRatio) OVER(PARTITION BY t00.newINDU) as thisYY_newINDU_CapitalAdequacyRatio,	-- 현월 자기자본비율
		SUM(t00.prevYY_CapitalAdequacyRatio) OVER(PARTITION BY t00.newINDU) as prevYY_newINDU_CapitalAdequacyRatio	-- 전년 동월 자기자본비율
	FROM
		(	
		SELECT 
			t0.newINDU,
