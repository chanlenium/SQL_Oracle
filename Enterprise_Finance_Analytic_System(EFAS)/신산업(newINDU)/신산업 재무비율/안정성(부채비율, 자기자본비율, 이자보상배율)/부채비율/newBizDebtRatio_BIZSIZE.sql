 ...
 * 활용 테이블 : TCB_NICE_FNST(재무제표), newGIUP_RAW(기업개요) -> DEBT_RATIO_TB 테이블 만듦
 *****************************************************/
-- 과거 데이터가 있으면 table 삭제하고 새로 생성
DROP TABLE IF EXISTS DEBT_RATIO_TB;
-- (Step1) 테이블 생성 월별, 산업별, 기업규모별 부채비율 테이블 생성 (최근 4개년 데이터)
-- (기준년월, 법인번호, 기업규모, 외감여부, 상장여부, KSIC, 신산업코드, 신산업명, 결산구분, 부채, 총자산, EFAS)
SELECT 
	t0.STD_YM,
	t0.CORP_NO,
	t0.COMP_SCL_DIVN_CD,
...
-- 결과 조회
SELECT * FROM DEBT_RATIO_TB ORDER BY STD_YM DESC;
...
-- 결과 조회
SELECT * FROM Cutoff_DEBT_RATIO_TB WHERE SEAC_DIVN = 'K' ORDER BY STD_YM DESC;



-- (Step4) 부채비율 연별 그래프 (대/중/소) (1: 대기업, 2: 중소기업, 3: 중견기업, 0: 대상아님)
DROP TABLE IF EXISTS Result_newINDU_DebtRatio_BIZSIZE;
SELECT DISTINCT
	t00.STD_YM,
	SUM(DEBT_RATIO_BIZ_1) OVER(PARTITION BY t00.STD_YM) as DEBT_RATIO_BIZ_1,
	SUM(DEBT_RATIO_BIZ_2) OVER(PARTITION BY t00.STD_YM) as DEBT_RATIO_BIZ_2,
	SUM(DEBT_RATIO_BIZ_3) OVER(PARTITION BY t00.STD_YM) as DEBT_RATIO_BIZ_3
	INTO Result_newINDU_DebtRatio_BIZSIZE
FROM
	(	
	SELECT 
		t0.*,
		DECODE(t0.BIZ_SIZE, '대기업', t0.DEBT_RATIO, 0) as DEBT_RATIO_BIZ_1,
		DECODE(t0.BIZ_SIZE, '중소기업', t0.DEBT_RATIO, 0) as DEBT_RATIO_BIZ_2,
		DECODE(t0.BIZ_SIZE, '중견기업', t0.DEBT_RATIO, 0) as DEBT_RATIO_BIZ_3
	FROM
		(
		SELECT DISTINCT 
			CASE
...
ORDER BY t00.STD_YM;
-- 결과 조회
SELECT * FROM Result_newINDU_DebtRatio_BIZSIZE ORDER BY STD_YM;
