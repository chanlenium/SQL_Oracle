...
-- 테이블 생성 (기준년월, 법인번호, 기업규모, KSIC, 신산업코드, 신산업명, 영업이익, 이자비용, EFAS)
SELECT 
  	t0.STD_YM,
	t0.CORP_NO,
	t0.COMP_SCL_DIVN_CD,
...
DROP TABLE IF EXISTS Result_newINDU_INTCOVRatio_INDU;
SELECT 	
	t000.*,
	thisYY_newINDU_INTCOVRATIO - prevYY_newINDU_INTCOVRATIO as "INCRATIO"	-- 전년 동월 대비 증감율
	INTO Result_newINDU_INTCOVRatio_INDU
FROM 
	(
	SELECT DISTINCT
		t00.newINDU,
		t00.thisYY,
		SUM(t00.thisYY_newINDU_INTCOVRATIO) OVER(PARTITION BY t00.newINDU) as thisYY_newINDU_INTCOVRATIO,-- 현월 이자보상배율
		SUM(t00.prevYY_newINDU_INTCOVRATIO) OVER(PARTITION BY t00.newINDU) as prevYY_newINDU_INTCOVRATIO	-- 전년 동월 이자보상배율
	FROM
		(	
		SELECT 
			t0.newINDU,
...
-- 결과 조회
SELECT * FROM Result_newINDU_INTCOVRatio_INDU ORDER BY TO_NUMBER(newINDU, '99');
