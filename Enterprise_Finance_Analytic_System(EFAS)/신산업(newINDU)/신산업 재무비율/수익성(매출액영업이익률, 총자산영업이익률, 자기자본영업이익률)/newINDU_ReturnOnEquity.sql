 ...
 * 활용 테이블 : newGIUP_RAW, TCB_NICE_FNST(재무제표) -> RETURN_ON_EQUITY_TB 테이블 만듦
 ****************************************************************/
-- (Standby) 월별, 산업별, 기업규모별 총자산영업이익률 테이블 RETURN_ON_ASSETS_TB 생성 
-- 과거 데이터가 있으면 table 삭제하고 새로 생성
DROP TABLE IF EXISTS RETURN_ON_EQUITY_TB;
-- 테이블 생성 (기준년월, 법인번호, 기업규모, KSIC, newINDU, newINDU_NM, 당기순이익, 자본금, EFAS)
SELECT 
  	t10.*, 
  	t20.EFIS as EFAS
...
FROM 
	...
	SELECT DISTINCT 
		...
		t.CapSTOCKS 
	FROM 
		RETURN_ON_EQUITY_TB t	-- 동일 법인번호에 newINDU가 여러개 매핑되는 경우 재무규모가 과대개상될 수 있어 이를 제거
	) t
ORDER BY 
  	t.STD_YM;
 ...
 * 활용 테이블 : RETURN_ON_EQUITY_TB -> INDU_RETURN_ON_EQUITY_TB 생성
 ****************************************************************/
-- 과거 데이터가 있으면 table 삭제하고 새로 생성
DROP TABLE IF EXISTS INDU_RETURN_ON_EQUITY_TB;
-- (Step1) 테이블 생성(업종코드, 당월 당기순이익, 당월 자기자본, 전년동월 당기순이익, 전년동월 자기자본
SELECT DISTINCT
  	t0.newINDU, 
  	t0.newINDU_NM,  
  	SUM(DECODE(t0.STD_YM, CONCAT('${inputYYYY}', '12'), INDU_NetINCOME, 0)) OVER(PARTITION BY t0.newINDU) as thisYY_INDU_NetINCOME,
  	SUM(DECODE(t0.STD_YM, CONCAT('${inputYYYY}', '12'), INDU_CapSTOCKS, 0)) OVER(PARTITION BY t0.newINDU) as thisYY_INDU_CapSTOCKS, 
  	SUM(DECODE(t0.STD_YM, CONCAT('${inputYYYY}' - 1, '12'), INDU_NetINCOME, 0)) OVER(PARTITION BY t0.newINDU) as prevYY_INDU_NetINCOME,
  	SUM(DECODE(t0.STD_YM, CONCAT('${inputYYYY}' - 1, '12'), INDU_CapSTOCKS, 0)) OVER(PARTITION BY t0.newINDU) as prevYY_INDU_CapSTOCKS 
  	INTO INDU_RETURN_ON_EQUITY_TB 
FROM 
  	(
    SELECT DISTINCT 
    	t.newINDU, 
    	t.newINDU_NM, 
      	t.STD_YM, 
      	SUM(t.NetINCOME) OVER(PARTITION BY t.newINDU, t.STD_YM) as INDU_NetINCOME,
      	SUM(t.CapSTOCKS) OVER(PARTITION BY t.newINDU, t.STD_YM) as INDU_CapSTOCKS 
    FROM 
...
-- 결과 조회
SELECT * FROM INDU_RETURN_ON_EQUITY_TB;


-- (Step 2) 총계 계산하여 insert
INSERT INTO INDU_RETURN_ON_EQUITY_TB 
SELECT 
  	'99', -- 총합은 INDU code '99'할당
  	'전체', 
  	SUM(t.thisYY_INDU_NetINCOME), 
  	SUM(t.thisYY_INDU_CapSTOCKS), 
  	SUM(t.prevYY_INDU_NetINCOME),
  	SUM(t.prevYY_INDU_CapSTOCKS) 
FROM 
  	INDU_RETURN_ON_EQUITY_TB t;
-- 결과 조회
SELECT * FROM INDU_RETURN_ON_EQUITY_TB t; 


-- (Step3) 당해년/전년 동월 자기자본이익률 및 전년 동월 대비 증가율 계산
SELECT 
  	t.*, 
  	ROUND(t.thisYY_INDU_NetINCOME / t.thisYY_INDU_CapSTOCKS, 4) as thisYY_RETURN_ON_EQUITY, 
  	ROUND(t.prevYY_INDU_NetINCOME / t.prevYY_INDU_CapSTOCKS, 4) as prevYY_RETURN_ON_EQUITY, 
  	ROUND(t.thisYY_INDU_NetINCOME / t.thisYY_INDU_CapSTOCKS - t.prevYY_INDU_NetINCOME / t.prevYY_INDU_CapSTOCKS, 4) as RETURN_ON_EQUITY_INC 
FROM 
  	INDU_RETURN_ON_EQUITY_TB t 
ORDER BY 
  	t.newINDU;