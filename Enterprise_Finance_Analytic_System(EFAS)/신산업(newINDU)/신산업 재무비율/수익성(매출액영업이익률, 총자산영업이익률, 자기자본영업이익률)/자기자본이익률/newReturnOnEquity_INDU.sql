			            ...
			            FROM 
			                ...
			                FROM 
			                    ...
			                    WHERE 
                      				...
                      				OR (t.REPORT_CD = '11' and t.ITEM_CD = '8900')	-- 자본총계(자기자본, 11/8900)
			                  	) t0 
			                WHERE 
...
-- (Step4) 자기자본이익률 연별 그래프 (업종별)
DROP TABLE IF EXISTS Result_newINDU_ReturnOnEquity_INDU;
SELECT 	
	t000.*,
	thisYY_newINDU_RETURN_ON_EQUITY_RATIO - prevYY_newINDU_RETURN_ON_EQUITY_RATIO as "INCRATIO"	-- 전년 동월 대비 증감율
	INTO Result_newINDU_ReturnOnEquity_INDU
FROM 
	(
	SELECT DISTINCT
		t00.newINDU,
		t00.thisYY,
		SUM(t00.thisYY_newINDU_RETURN_ON_EQUITY_RATIO) OVER(PARTITION BY t00.newINDU) as thisYY_newINDU_RETURN_ON_EQUITY_RATIO,	-- 현월 자기자본이익률
		SUM(t00.prevYY_newINDU_RETURN_ON_EQUITY_RATIO) OVER(PARTITION BY t00.newINDU) as prevYY_newINDU_RETURN_ON_EQUITY_RATIO	-- 전년 동월 자기자본이익률
	FROM
		(	
		SELECT 
			t0.newINDU,
		  	...
		  	CASE	
		  		WHEN '${isSangJang}' = 'Y' 
		  		THEN DECODE(t0.STD_YM, t0.thisYY, t0.RETURN_ON_EQUITY_RATIO, 0)
		  		ELSE DECODE(CONCAT(t0.STD_YM, '12'), t0.thisYY, t0.RETURN_ON_EQUITY_RATIO, 0)
		  	END as "thisYY_newINDU_RETURN_ON_EQUITY_RATIO",
		  	CASE	
		  		WHEN '${isSangJang}' = 'Y' 
		  		THEN DECODE(t0.STD_YM, t0.prevYY, t0.RETURN_ON_EQUITY_RATIO, 0)
		  		ELSE DECODE(CONCAT(t0.STD_YM, '12'), t0.prevYY, t0.RETURN_ON_EQUITY_RATIO, 0)
		  	END as "prevYY_newINDU_RETURN_ON_EQUITY_RATIO"
		FROM
			(
			SELECT DISTINCT 
				t.newINDU,
...
-- 결과 조회
SELECT * FROM Result_newINDU_ReturnOnEquity_INDU ORDER BY TO_NUMBER(newINDU, '99');
