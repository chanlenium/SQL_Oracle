...
-- (기준년월, 법인번호, 기업규모, 외감여부, 상장여부, KSIC, 신산업코드, 신산업명, 결산구분, 당기순이익, 자기자본, 자기자본이익률, EFAS)
SELECT 
	t0.STD_YM,
	t0.CORP_NO,
	t0.COMP_SCL_DIVN_CD,
...
DROP TABLE IF EXISTS Result_newINDU_ReturnOnEquity_BIZSIZE;
SELECT DISTINCT
	t00.STD_YM,
	...
	INTO Result_newINDU_ReturnOnEquity_BIZSIZE
FROM
	(	
	SELECT 
		t0.*,
...
ORDER BY t00.STD_YM;
-- 결과 조회
SELECT * FROM Result_newINDU_ReturnOnEquity_BIZSIZE ORDER BY STD_YM;
