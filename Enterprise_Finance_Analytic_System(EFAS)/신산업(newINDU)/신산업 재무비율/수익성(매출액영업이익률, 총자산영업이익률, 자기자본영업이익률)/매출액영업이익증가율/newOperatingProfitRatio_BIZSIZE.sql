...
DROP TABLE IF EXISTS Result_newINDU_OperatingProfitRatio_BIZSIZE;
SELECT DISTINCT
	t00.STD_YM,
	SUM(OPERATING_PROFIT_RATIO_BIZ_1) OVER(PARTITION BY t00.STD_YM) as OPERATING_PROFIT_RATIO_BIZ_1,
	SUM(OPERATING_PROFIT_RATIO_BIZ_2) OVER(PARTITION BY t00.STD_YM) as OPERATING_PROFIT_RATIO_BIZ_2,
	SUM(OPERATING_PROFIT_RATIO_BIZ_3) OVER(PARTITION BY t00.STD_YM) as OPERATING_PROFIT_RATIO_BIZ_3
	INTO Result_newINDU_OperatingProfitRatio_BIZSIZE
FROM
	(	
	SELECT 
		t0.*,
		DECODE(t0.BIZ_SIZE, '대기업', t0.OPERATING_PROFIT_RATIO, 0) as OPERATING_PROFIT_RATIO_BIZ_1,
		DECODE(t0.BIZ_SIZE, '중소기업', t0.OPERATING_PROFIT_RATIO, 0) as OPERATING_PROFIT_RATIO_BIZ_2,
		DECODE(t0.BIZ_SIZE, '중견기업', t0.OPERATING_PROFIT_RATIO, 0) as OPERATING_PROFIT_RATIO_BIZ_3
	FROM
		(
		SELECT DISTINCT 
			CASE
...
ORDER BY t00.STD_YM;
-- 결과 조회
SELECT * FROM Result_newINDU_OperatingProfitRatio_BIZSIZE ORDER BY STD_YM;
