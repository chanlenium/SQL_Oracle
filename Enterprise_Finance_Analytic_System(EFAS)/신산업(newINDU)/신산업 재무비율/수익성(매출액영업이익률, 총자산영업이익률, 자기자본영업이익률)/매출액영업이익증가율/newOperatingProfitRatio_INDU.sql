...
DROP TABLE IF EXISTS Result_newINDU_OperatingProfitRatio_INDU;
SELECT 	
	t000.*,
	thisYY_newINDU_OPERATING_PROFIT_RATIO - prevYY_newINDU_OPERATING_PROFIT_RATIO as "INCRATIO"	-- 전년 동월 대비 증감율
	INTO Result_newINDU_OperatingProfitRatio_INDU
FROM 
	(
	SELECT DISTINCT
		t00.newINDU,
		t00.thisYY,
		SUM(t00.thisYY_newINDU_OPERATING_PROFIT_RATIO) OVER(PARTITION BY t00.newINDU) as thisYY_newINDU_OPERATING_PROFIT_RATIO,	-- 현월 매출액 영업이익률
		SUM(t00.prevYY_newINDU_OPERATING_PROFIT_RATIO) OVER(PARTITION BY t00.newINDU) as prevYY_newINDU_OPERATING_PROFIT_RATIO	-- 전년 동월 매출액 영업이익률
	FROM
		(	
		SELECT 
			t0.newINDU,
		  	...
		  	END as "thisYY_newINDU_OPERATING_PROFIT_RATIO",
		  	CASE	
		  		WHEN '${isSangJang}' = 'Y' 
		  	...
		  	END as "prevYY_newINDU_OPERATING_PROFIT_RATIO"
		FROM
			(
			SELECT DISTINCT 
				t.newINDU,
...
-- 결과 조회
SELECT * FROM Result_newINDU_OperatingProfitRatio_INDU ORDER BY TO_NUMBER(newINDU, '99');
