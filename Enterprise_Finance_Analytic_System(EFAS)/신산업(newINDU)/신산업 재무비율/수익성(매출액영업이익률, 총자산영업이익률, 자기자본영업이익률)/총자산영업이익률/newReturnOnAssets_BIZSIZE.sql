...
-- (기준년월, 법인번호, 기업규모, 외감여부, 상장여부, KSIC, 신산업코드, 신산업명, 결산구분, 영업이익(손실), 총자산, 총자산 영업이익률, EFAS)
SELECT 
	t0.STD_YM,
	t0.CORP_NO,
	t0.COMP_SCL_DIVN_CD,
...
-- (Step4) 총자산 영업이익률 연별 그래프 (대/중/소) (1: 대기업, 2: 중소기업, 3: 중견기업, 0: 대상아님)
DROP TABLE IF EXISTS Result_newINDU_ReturnOnAssets_BIZSIZE;
SELECT DISTINCT
	t00.STD_YM,
	SUM(RETURN_ON_ASSETS_RATIO_BIZ_1) OVER(PARTITION BY t00.STD_YM) as RETURN_ON_ASSETS_RATIO_BIZ_1,
	SUM(RETURN_ON_ASSETS_RATIO_BIZ_2) OVER(PARTITION BY t00.STD_YM) as RETURN_ON_ASSETS_RATIO_BIZ_2,
	SUM(RETURN_ON_ASSETS_RATIO_BIZ_3) OVER(PARTITION BY t00.STD_YM) as RETURN_ON_ASSETS_RATIO_BIZ_3
	INTO Result_newINDU_ReturnOnAssets_BIZSIZE
FROM
	(	
	SELECT 
		t0.*,
		DECODE(t0.BIZ_SIZE, '대기업', t0.RETURN_ON_ASSETS_RATIO, 0) as RETURN_ON_ASSETS_RATIO_BIZ_1,
		DECODE(t0.BIZ_SIZE, '중소기업', t0.RETURN_ON_ASSETS_RATIO, 0) as RETURN_ON_ASSETS_RATIO_BIZ_2,
		DECODE(t0.BIZ_SIZE, '중견기업', t0.RETURN_ON_ASSETS_RATIO, 0) as RETURN_ON_ASSETS_RATIO_BIZ_3
	FROM
		(
		SELECT DISTINCT 
			CASE
...
ORDER BY t00.STD_YM;
-- 결과 조회
SELECT * FROM Result_newINDU_ReturnOnAssets_BIZSIZE ORDER BY STD_YM;
